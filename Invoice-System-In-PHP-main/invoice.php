...
class invoicr extends FPDF_rotation 
{
	...
	var $language; // Language setting for invoice
	...
	// Properties that are set dynamically - declared for PHP 8.2+ compatibility
	var $currency;
	...
	*******************************************************************************/
	...
	{
		$this->invoicr($size, $currency, $language);
	}
	...
	function setLogo($logo=0,$maxWidth=0,$maxHeight=0)
	{
		...
		// Normalize and resolve the logo path to improve portability on Windows/Linux
		$resolved = $this->resolveImagePath($logo);
		$this->logo = $resolved ? $resolved : $logo;
		$this->dimensions = $this->resizeToFit($this->logo);
	}
	...
	*******************************************************************************/
	function Header()
	{
		...
		$title = isset($this->title) ? $this->title : 'Invoice';
	    $this->Cell(0,5,iconv("UTF-8", "ISO-8859-1",strtoupper($title)),0,1,'R');
		...
		if($this->PageNo()==1) 
		{
			...
			$this->SetTextColor(50,50,50);
			$this->SetFont($this->font,'B',10);
			$from_name = (isset($this->from) && isset($this->from[0])) ? $this->from[0] : '';
			$to_name = (isset($this->to) && isset($this->to[0])) ? $this->to[0] : '';
			$ship_name = (isset($this->ship) && isset($this->ship[0])) ? $this->ship[0] : '';
			$this->Cell($width,$lineheight,$from_name,0,0,'L');
			$this->Cell($width,$lineheight,$to_name,0,0,'L');
			$this->Cell(0,$lineheight,$ship_name,0,0,'L'); // ADDED SHIPPING
			$this->SetFont($this->font,'',8);
			$this->SetTextColor(100,100,100);
			$this->Ln(7);
			$from_count = (isset($this->from) && is_array($this->from)) ? count($this->from) : 0;
			$to_count = (isset($this->to) && is_array($this->to)) ? count($this->to) : 0;
			$ship_count = (isset($this->ship) && is_array($this->ship)) ? count($this->ship) : 0;
			for($i=0; $i<max($from_count, $to_count, $ship_count); $i++) { // ADDED SHIPPING
				$from_addr = (isset($this->from[$i])) ? $this->from[$i] : '';
				$to_addr = (isset($this->to[$i])) ? $this->to[$i] : '';
				$ship_addr = (isset($this->ship[$i])) ? $this->ship[$i] : '';
				$this->Cell($width,$lineheight,iconv("UTF-8", "ISO-8859-1",$from_addr),0,0,'L');
				$this->Cell($width,$lineheight,iconv("UTF-8", "ISO-8859-1",$to_addr),0,0,'L');
				$this->Cell(0,$lineheight,iconv("UTF-8", "ISO-8859-1",$ship_addr),0,0,'L'); // ADDED SHIPPING
				$this->Ln(5);
			}	
			$this->Ln(-6);
		}
	...
	*******************************************************************************/
	private function setLanguage($language)
	{
		...
		$language_file = 'languages/'.$language.'.inc';
	...
	{
		// Some environments require an absolute path or forward slashes for PNGs
		$path = is_string($image) ? $image : '';
		$info = @getimagesize($path);
		if($info === false) {
			// Fallback: try to resolve again with forward slashes
			$alt = str_replace('\\','/',$path);
			$info = @getimagesize($alt);
		}
		if($info === false) {
			// As a final fallback, use a reasonable default size
			$width = 300; // px
			$height = 90; // px
		} else {
			$width = $info[0];
	...
	// Attempt to resolve an image path robustly across environments
	private function resolveImagePath($logo)
	{
		if(!$logo) return $logo;
		// If already readable, return normalized path
		if(@is_readable($logo)) {
			return str_replace('\\','/',$logo);
		}
		// Try path relative to this file
		$rel = __DIR__ . '/' . ltrim($logo,'/');
		if(@is_readable($rel)) {
			return str_replace('\\','/',$rel);
		}
		// If a dedicated PDF logo constant exists, prefer it
		if(defined('COMPANY_LOGO_PDF') && @is_readable(COMPANY_LOGO_PDF)) {
			return str_replace('\\','/', COMPANY_LOGO_PDF);
		}
		return $logo; // give back original; downstream will use fallbacks
	}
